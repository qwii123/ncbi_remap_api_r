...
MainSubmission = function(mode,verbose,URL.SERVICE,URL.CHECK,FILE.DATA,out_mode,FILE.OUT,ID.BATCH,acc_one,acc_two,min_cov,max_exp,allow_dupes,merge,in_format,out_format){
  ...
  if(out_mode=="pass"){
    xls.file = download.list[2]
    httr::GET(xls.file,httr::write_disk(tf <- tempfile(fileext=".xls")))
    return(read.table(tf,sep="\t",quote="",header=FALSE,fill=TRUE))
  }
}
...
RecheckSubmission = function(check.URL){
  repeat{
    ...
    if(xml.title=="NCBI Remap: Results"){
      ...
      elapsed = xml.doc %>% html_nodes("div.grid") %>% html_nodes("div#remapTemplateDiv") %>% html_nodes("div#content") %>% html_nodes("h1") %>% html_text
      elapsed = gsub("\n","",elapsed)
